...
#   - libvicecore.dylib and libvicecore_vsid.dylib are committed prebuilt
#     binaries (see docs/NATIVE_BUILD.md) and were cross-compiled at 13.0.
#     Rebuilding them to change one load command would need the cross-compiled
#     VICE object tree, which is not in this repository.
#
# So both bare dylibs and .framework bundles are patched. The dylibs are the
# reason this differs from a stock Flutter project: the cores are dlopen'd by
# absolute path out of Frameworks/ rather than linked, so they arrive as loose
# Mach-O files with no Info.plist of their own.
#
