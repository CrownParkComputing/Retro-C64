...
# whose size Apple accepts -- iPhone 17 Pro Max (1320x2868) covers the 6.9"
...
BUNDLE_ID="${SHOT_BUNDLE_ID:-com.crownparkcomputing.c64-retro}"
SEED="${SHOT_SEED:-}"
SEED_SUPPORT="${SHOT_SEED_SUPPORT:-}"
...
seeder() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    ...
    if [ -n "$c" ] && [ "$c" != "$last" ]; then
      ...
      # The ROMs do NOT live in Documents. They are Commodore's and are not
      # bundled, and the emulator reads them from the app support directory --
      # <support>/vice/C64 and <support>/vice/DRIVES. Without them the app
      # falls back to free-ROM mode, which deliberately hides most of the rail,
      # so a screenshot run staged only with games captures a four-entry rail
      # and cannot reach Paths, Video, Input, Core, Music or History at all.
      if [ -n "$SEED_SUPPORT" ] && [ -d "$SEED_SUPPORT" ]; then
        mkdir -p "$c/Library/Application Support"
        for e in "$SEED_SUPPORT"/*; do
          [ -e "$e" ] || continue
          ditto "$e" "$c/Library/Application Support/$(basename "$e")"
        done
        echo "seeded $c/Library/Application Support"
      fi
      last="$c"
    fi
    # Fast poll, deliberately. The container appears at INSTALL time and the
    # app launches moments later, and this app decides at STARTUP whether it
    # has ROMs -- with none present it switches to free-ROM mode, which hides
    # most of the rail and cannot be undone without restarting. So the fixtures
    # have to win that race, not merely arrive eventually.
    sleep 0.2
  done
}
