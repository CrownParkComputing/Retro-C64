...
class EmulatorScreen extends StatefulWidget {
  ...
  /// Called when the user touches the picture itself - not the on-screen
  /// controls. The workbench uses it to bring its auto-hidden chrome back;
  /// a stick move or a fire press deliberately does not.
...
class _EmulatorScreenState extends State<EmulatorScreen> {
  ...
  Future<void> _addCustomButton() async {
    ...
    widget.onCustomButtonsChanged?.call([...widget.customButtons, binding]);
  }

  void _updateJoystick() => widget.core.joystick(
    widget.joystickPort,
    _dirMask | _fireMask | _padMask,
  );
  ...
  final EmulatorUiState _fallbackUi = EmulatorUiState();
  EmulatorUiState get _ui => widget.ui ?? _fallbackUi;
      ...
      builder: (context, _) => Scaffold(
        ...
        body: Column(
          children: [
            Expanded(
              child: Stack(
                children: [
                  ...
                  Positioned.fill(
                    child: Center(child: FramebufferView(core: widget.core)),
                  ),
                  ...
                  // The chrome's wake-up call. UNDER the pad and the
                  // keyboard, so a touch they claim never reaches it, and
                  // only a touch on the picture itself brings the rail and
                  // the strip back. Translucent, so the picture's own touch
                  // handling underneath still sees the same event.
                  if (widget.onScreenTouched != null)
                    Positioned.fill(
                      child: Listener(
                        behavior: HitTestBehavior.translucent,
                        onPointerDown: (_) => widget.onScreenTouched!(),
                      ),
                    ),
                            ...
                            return Stack(
                              children: [
                                _MovableControl(
                                  ...
                                  editing: _ui.editingLayout,
                                  label: 'Joystick',
                                  onMoved: (f) =>
                                      _moveControl(kControlIdStick, f),
                                  onMoveEnd: () =>
                                      _commitControlPosition(kControlIdStick),
                                  child: switch (_joystickStyle) {
                                    JoystickStyle.wobble => WobbleJoystick(
                                      onDirections: _joystickDirections,
                                    ),
                                    JoystickStyle.dpad => DpadView(
                                      onDirections: _joystickDirections,
                                    ),
                                  },
                                ),
                                _MovableControl(
                                  ...
                                  label: 'Buttons',
                                  onMoved: (f) =>
                                      _moveControl(kControlIdButtons, f),
                                  onMoveEnd: () =>
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      for (final binding
                                          in widget.customButtons) ...[
                                        CustomKeyButton(
                                          binding: binding,
                                          core: widget.core,
                                          onDirectionChanged:
                                              _setDirectionButtonBit,
                                        ),
                                        const SizedBox(height: 8),
                                      ],
                                      ActionButton(
                                        label: 'A',
                                        onFireBitChanged: (down) => _setFireBit(
                                          ViceJoyBits.fire1,
                                          down,
                                        ),
                                      ),
                  ...
                  // No filename over the picture. The workbench's status strip along
                  // the bottom of the window already names what is loaded, and a
                  // second copy floating in the top letterbox band was the same
                  // string twice, one of them on top of the game.
            ...
            // covering the game.
            if (_ui.keyboardVisible)
              C64KeyboardOverlay(
                core: widget.core,
                onClose: () => _ui.keyboardVisible = false,
              ),
          ],
        ),
      ),
        ...
        child: editing
            ? GestureDetector(
                ...
                onPanUpdate: (d) {
                  ...
                  onMoved(
                    Offset(
                      (fraction.dx + d.delta.dx / area.width).clamp(
                        _minFraction,
                        _maxFraction,
                      ),
                      (fraction.dy + d.delta.dy / area.height).clamp(
                        _minFraction,
                        _maxFraction,
                      ),
                    ),
